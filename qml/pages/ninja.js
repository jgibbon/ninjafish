...
// 			options.url = options.url + 'access_token=' + options.qs.access_token + '&';
// 		if (options.qs.user_access_token)
// 			options.url = options.url + 'user_access_token=' + options.qs.user_access_token;
// 	}
...
// 	xmlhttp.setRequestHeader('Accept', 'application/json'); // Tells server that this call is made for ajax purposes.
// 	// Most libraries like jQuery/Prototype/Dojo do this
// 	xmlhttp.send(null); // No data need to send along with the request.
// }

var utils = {
	filter: function(filterObj, set) {
	...
	findSubDevice: function(filterObj, set) {
		var out = {};
		for (var i in set) {
			if (set[i].hasOwnProperty('subDevices')) {
				...
				var filterMatched = 0;
				var filterLength = Object.keys(filterObj).length;
				var returned = utils.filter(filterObj, set[i].subDevices);
