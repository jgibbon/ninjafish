...
function request(options, callback) {
	...
	if (options.qs) {
        options.uri = options.uri + '?access_token=' + options.qs.access_token;
	}
	...
	request.log.debug('setting data', options);
...
function getLogger() {
	var logger = {}, levels = ['trace', 'debug', 'info', 'warn', 'error'],
		level, i
...
function is_crossDomain(url) {
	...
	var ajaxLocParts = rurl.exec(ajaxLocation.toLowerCase()) || [],
		...
		var result = !! (
			parts &&
			(parts[1] != ajaxLocParts[1] || parts[2] != ajaxLocParts[2] || (parts[3] || (parts[1] === "http:" ? 80 : 443)) != (ajaxLocParts[3] || (ajaxLocParts[1] === "http:" ? 80 : 443)))
	)
